...
# ##-- efficient create masks function
...
#     mask_positions_x = torch.randint(0, image_size - mask_size, (n, batch_size))
#     mask_positions_y = torch.randint(0, image_size - mask_size, (n, batch_size))

#     # Initialize a tensor to hold masks for all images in the batch
#     masks = torch.zeros((n, batch_size, 3, image_size, image_size), device=device)

#     # Set random square regions to 1 in each mask for each image
#     for j in range(n):
#         x, y = mask_positions_x[j], mask_positions_y[j]
#         masks[j, torch.arange(batch_size), :, x:x+mask_size, y:y+mask_size] = 1
#     return masks

# def create_masks_from_batch(batch_images, n=8, divisor=16):
#     # print(f"received batch size ==> {batch_images.size()}")
#     batch_size, _, image_size, _ = batch_images.size()
#     device=batch_images.device
#     mask_size = image_size // divisor
...
#             mask = torch.zeros((batch_size, 3, image_size, image_size)).to(device)  # Initialize mask for 3 layers
#             # mask=torch.zeros((batch_size, 3, image_size, image_size), dtype=torch.float32, device=device)
#             x, y = mask_positions_x[i, j], mask_positions_y[i, j]
#             mask[i, :, x:x+mask_size, y:y+mask_size] = 1
#         masks_list.append(mask)
#     return masks_list

# def create_batch_mask(batch_images, n=1,divisor=16):
#     batch_size, _, image_size, _ = batch_images.size()
#     # device = batch_images.device
#     device="cpu"
#     mask_size = image_size // divisor
#     # Initialize a list to hold two masks for each image
#     masks_list = []
...
def get_feature_map(model,batched_images,batch_collected_masks,targets):
    # print(f"len recv__batched_mask--> {len(batch_collected_masks)}")
    # print(f"recv__batched_mask--> {batch_collected_masks[0].size()}")
    # print(f"recv batch--mask unique--> {torch.unique(batch_collected_masks[0], return_counts=True)}")
    # print(f"image unique--> {torch.unique(batched_images, return_counts=True)}")
    # print(f"recv__batched_mask--> {batch_collected_masks}")
    # print(f"batched_imgs--> {batched_images.size()}")
    batch_size, _, image_size, _ = batched_images.size()
    feature_map_list=[]
    for single_batch_mask in batch_collected_masks:
        ## make it boolean mask
        single_batch_mask=single_batch_mask.bool()
        # masked_image=batched_images*single_batch_mask
        feature_map=model(batched_images,targets)
...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        LOGGER.info(('\n' + '%11s' * 5+'%15s'+'%11s' * 2) % ('Epoch', 'GPU_mem', 'box_loss', 'obj_loss', 'cls_loss','custom_loss', 'Instances', 'Size'))
        if RANK in {-1, 0}:
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            with torch.cuda.amp.autocast(amp):
                ...
                if opt.teacher_weight:
                    ...
                    # pred, student_feature = model(imgs, target=targets)
                    # teacher_preds, teacher_feature = teacher_model(imgs, target=targets)
                    # print(f"student feature ==> {features.size()}")
                    
                    student_adapted_feature_maps_list=[]
                    for std_feat_m in student_feature_maps:
                        student_adapted_feature_maps_list.append(stu_feature_adapt(std_feat_m))

                    loss, loss_items = compute_loss(pred, targets, None, None,teacher_feature_maps,student_adapted_feature_maps_list)  # loss scaled by batch_size
                else:
